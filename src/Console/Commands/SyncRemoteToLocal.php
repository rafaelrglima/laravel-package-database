...
class SyncRemoteToLocal extends Command
{
    ...
    public function handle()
    {
        ...
        exec("ssh -t {$stg_ssh_hostname} \"docker exec -it {$stg_docker_hostname} bash -c 'mysqldump -u{$stg_database_name} -p{$stg_database_pw} --events --routines --triggers | gzip > /home/{$temp_filename}'\"");
        ...
        $this->info("6. Dropping local database: {$stg_database_name}_bkp");
        exec("mysql -h172.17.0.2 -uroot -p@rtNdX34d7882 -e 'drop database {$stg_database_name}_bkp;'");

        $this->info("7. Creating local database: {$stg_database_name}_bkp");
        exec("mysql -h172.17.0.2 -uroot -p@rtNdX34d7882 -e 'create database {$stg_database_name}_bkp;'");
